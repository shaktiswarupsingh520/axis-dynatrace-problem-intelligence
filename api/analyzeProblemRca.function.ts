...
const text = (value: unknown): string => {
  ...
  if (typeof value === 'object') {
    ...
    for (const key of ['name', 'entityName', 'displayName', 'id', 'entityId']) { const candidate = text(record[key]); if (candidate) return candidate; }
  }
...
const entityText = (value: unknown): string => {
  if (value == null) return '';
  if (typeof value === 'string') return value;
  if (Array.isArray(value)) { for (const item of value) { const found = entityText(item); if (found) return found; } return ''; }
  if (typeof value === 'object') {
    const record = value as Row;
    for (const key of ['name', 'entityName', 'displayName', 'entity.name', 'id', 'entityId']) { const found = entityText(record[key]); if (found) return found; }
  }
  return '';
};
...
async function dql(query: string, max = 200): Promise<Row[]> { const response = await queryExecutionClient.queryExecute({ body: { query, requestTimeoutMilliseconds: 30000, maxResultRecords: max } }); let result = response.result; const token = response.requestToken; for (let attempt = 0; !result && token && attempt < 30; attempt += 1) { const poll = await queryExecutionClient.queryPoll({ requestToken: token, requestTimeoutMilliseconds: 30000 }); result = poll.result; if (!result) await new Promise<void>((resolve) => setTimeout(resolve, 300)); } if (!result) throw new Error('DQL did not complete.'); return normalizeRows(result.records); }
async function safeDql(query: string, max = 200): Promise<Row[]> { try { return await dql(query, max); } catch (error) { void error; return []; } }
...
async function loadEvidence(id: string): Promise<Evidence> {
  ...
  const start = text(problem['event.start']); const end = text(problem['event.end']) || new Date().toISOString();
  const eventQuery = eventList ? `fetch dt.davis.events, from:now()-365d, to:now()\n| filter in(event.id,array(${eventList}))\n| fields event.id,event.name,event.type,event.status,event.severity,event.category,event.start,event.end,event.description,dt.source_entity,dt.smartscape_source.id,dt.smartscape_source.type,dt.davis.is_rootcause_relevant,root_cause_entity_id,root_cause.smartscape_entity\n| sort event.start asc\n| limit 80` : '';
  ...
  const historyQuery = `fetch dt.davis.problems, from:now()-30d, to:now()\n| filter not(dt.davis.is_duplicate) and event.name == "${q(text(problem['event.name']))}"${zoneFilter}\n| fields display_id,event.name,event.status,event.severity,event.start,event.end,event.category,root_cause.smartscape_entity,root_cause_entity_id,dt.davis.affected_users_count\n| sort event.start desc\n| limit 100`;
  const snapshotQuery = text(problem['event.id']) ? `fetch dt.davis.problems.snapshots, from:now()-365d, to:now()\n| filter event.id == "${q(text(problem['event.id']))}"\n| fields timestamp,event.status,event.status_transition,event.severity,event.name,root_cause_entity_id,root_cause.smartscape_entity\n| sort timestamp asc\n| limit 50` : '';
  const [events, logs, history, snapshots] = await Promise.all([eventQuery ? safeDql(eventQuery, 80) : Promise.resolve([] as Row[]), logQuery ? safeDql(logQuery, 25) : Promise.resolve([] as Row[]), safeDql(historyQuery, 100), snapshotQuery ? safeDql(snapshotQuery, 50) : Promise.resolve([] as Row[])]);
  const occurrenceCount = history.length;
  const occurrences: Occurrence[] = history.map((row): Occurrence => ({ problemId: text(row.display_id), title: text(row['event.name']), status: text(row['event.status']), severity: text(row['event.severity']), start: text(row['event.start']), end: text(row['event.end']), duration: duration(text(row['event.start']), text(row['event.end'])) })).filter((occurrence) => Boolean(occurrence.problemId));
...
function fallbackRca(id: string, evidence: Evidence): string {
  const p = evidence.problem; const title = text(p['event.name']) || 'Dynatrace problem';
  const root = entityText(p['root_cause.smartscape_entity']) || entityText(p.root_cause_entity_id) || entityText(evidence.events.find((event) => event['dt.davis.is_rootcause_relevant'] === true)?.root_cause?.toString?.());
  const relevant = evidence.events.find((event) => event['dt.davis.is_rootcause_relevant'] === true);
  const signal = text(relevant?.['event.description']) || text(relevant?.['event.name']) || text(p['event.description']);
  const affected = text(p.affected_entity_names) || text(p.affected_entity_ids) || 'affected entity not returned'; const scope = evidence.managementZones.length ? evidence.managementZones.join(', ') : 'Management zone could not be derived from the problem entities';
  return `ROOT CAUSE ANALYSIS\n\nProblem: ${id}\nTitle: ${title}\nStatus: ${text(p['event.status']) || 'Not available'}\nSeverity: ${text(p['event.severity']) || 'Not available'}\nManagement zone scope: ${scope}\nRoot-cause entity: ${root || 'Not proven by available evidence'}\n\nExecutive Summary\n${signal || 'No causal signal was returned by Davis.'}\n\nRoot Cause Assessment\n${root ? `Davis identified ${root} as a root-cause entity.` : 'Davis did not return a definitive root-cause entity. The following assessment is based on available evidence.'} ${signal || ''}\n\nTechnical Root-Cause Chain\nProblem → ${signal || 'Davis event evidence'} → ${affected}\n\nIncident Timeline\n${text(p['event.start']) || 'Start not returned'} → ${text(p['event.end']) || 'Open / end not returned'}\n\nPast Occurrences & Recurrence Pattern\n${evidence.occurrenceCount} matching occurrences in the last 30 days within the derived management-zone scope.\n\nImpact Assessment\nImpact level: ${text(p['dt.davis.impact_level']) || 'Not available'}; affected users: ${text(p['dt.davis.affected_users_count']) || 'Not available'}\n\nImmediate Remediation Plan\nValidate the root-cause signal on the affected entity, review incident-window logs, and confirm recovery in Davis before closing the incident.\n\nPermanent / Preventive Actions\nInstrument the affected dependency, stabilize the triggering component, and add alerting for the observed failure pattern.\n\nMonitoring & Alerting Recommendations\nMonitor the affected service, root-cause entity, error rate, response time and recurrence frequency.\n\nValidation Checklist\nConfirm root cause, validate dependency chain, correlate metrics and logs, verify remediation, and monitor for recurrence.\n\nRCA Confidence & Evidence Gaps\n${root && signal ? 'Medium' : 'Low'} — no unsupported root cause is asserted.`;
}

async function ask(id: string, evidence: Evidence): Promise<{ analysis: string; assistFallback: boolean }> {
  const p = evidence.problem;
  const compactEvidence = JSON.stringify({ problem: { id, title: text(p['event.name']), status: text(p['event.status']), severity: text(p['event.severity']), category: text(p['event.category']), start: text(p['event.start']), end: text(p['event.end']), description: text(p['event.description']), rootCause: entityText(p['root_cause.smartscape_entity']) || entityText(p.root_cause_entity_id), impact: text(p['dt.davis.impact_level']), affectedUsers: text(p['dt.davis.affected_users_count']), affectedEntities: text(p.affected_entity_names), managementZones: evidence.managementZones, occurrenceCount: evidence.occurrenceCount }, timeline: evidence.snapshots.slice(0, 20), correlatedEvents: evidence.events.slice(0, 25), incidentLogs: evidence.logs.slice(0, 12), pastOccurrences: evidence.history.slice(0, 20) }).slice(0, 9800);
  const prompt = `Create a customer-ready incident RCA for Davis Problem ${id}. Use ONLY the supplied evidence. Never invent facts. Clearly distinguish observed facts from inference. If a cause is not proven, say "Not proven by available evidence". The recurrence scope is authoritative: ONLY the last 30 days and ONLY the management-zone scope listed in the evidence. The field occurrenceCount is the authoritative total number of matching occurrences. The pastOccurrences array is only a display sample and may be truncated; NEVER use its array length as the total. Return sections: Executive Summary; Incident Overview; Root Cause Assessment; Technical Root-Cause Chain; Incident Timeline; Past Occurrences & Recurrence Pattern; Impact Assessment; Immediate Remediation Plan; Permanent / Preventive Actions; Monitoring & Alerting Recommendations; Validation Checklist; RCA Confidence & Evidence Gaps.\n\nEVIDENCE:\n${compactEvidence}`;
  try { const response = await publicClient.recommenderConversation({ body: { text: prompt, context: [{ type: 'document-retrieval', value: 'disabled' }, { type: 'supplementary', value: compactEvidence }, { type: 'instruction', value: 'Analyze the supplied Dynatrace evidence directly. Do not respond with an access limitation.' }], annotations: { origin: 'Axis Davis Capacity Planner RCA', problemId: id } } }); const answer = extractAssistText(response); if (answer) return { analysis: answer, assistFallback: false }; } catch (error) { void error; }
...
export default async function (payload: AnalyzePayload) {
  ...
  const evidence = await loadEvidence(payload.problemId); const result = await ask(payload.problemId, evidence); const p = evidence.problem;
  const nativeRootCauseEntity = entityText(p['root_cause.smartscape_entity']) || entityText(p.root_cause_entity_id) || entityText(evidence.events.find((event) => event['dt.davis.is_rootcause_relevant'] === true)?.root_cause?.toString?.()) || null;
  return { problemId: payload.problemId, analysis: result.analysis, generatedAt: new Date().toISOString(), nativeRootCauseEntity, definitiveRootCause: Boolean(nativeRootCauseEntity), assistFallback: result.assistFallback, problemFacts: { title: text(p['event.name']) || 'Dynatrace problem', status: text(p['event.status']) || 'Not available', severity: text(p['event.severity']) || 'Not available', category: text(p['event.category']) || 'Not available', start: text(p['event.start']), end: text(p['event.end']), duration: duration(text(p['event.start']), text(p['event.end'])), impactLevel: text(p['dt.davis.impact_level']) || 'Not available', affectedUsers: text(p['dt.davis.affected_users_count']) || 'Not available', affectedEntities: text(p.affected_entity_names) || text(p.affected_entity_ids) || 'Not available' }, evidenceSummary: { correlatedEvents: evidence.events.length, incidentLogs: evidence.logs.length, historicalOccurrences: evidence.occurrenceCount, timelineSnapshots: evidence.snapshots.length }, occurrenceCount: evidence.occurrenceCount, occurrences: evidence.occurrences, managementZones: evidence.managementZones, recurrenceWindow: '30d' };
}
