...
async function dql(query: string, max = 200): Promise<Row[]> {
  ...
  return (queryResult.records ?? []).filter(Boolean) as Row[];
}
...
async function askAssist(problemId: string, evidence: Evidence): Promise<{ text: string; status: string }> {
  ...
  const context = JSON.stringify({
    problem: {
      ...
      start: text(p['event.start']), end: text(p['event.end']), durationMinutes: durationMinutes(text(p['event.start']), text(p['event.end'])), description: text(p['event.description']),
      nativeRootCauseEntity: text(p['root_cause.smartscape_entity']) || text(p.root_cause_entity_id), impact: text(p['dt.davis.impact_level']), affectedUsers: text(p['dt.davis.affected_users_count']),
      analysisReady: text(p['dt.analysis.ready']), affectedEntityIds: text(p.affected_entity_ids),
    },
    timeline: evidence.snapshots.slice(0, 60),
    correlatedEvents: evidence.events.slice(0, 90),
    pastOccurrences: evidence.history.slice(0, 35),
    relatedProblems: evidence.related.slice(0, 35),
    incidentLogs: evidence.logs.slice(0, 100),
    nearbyDeployments: evidence.deployments.slice(0, 40),
  }).slice(0, 45000);
  ...
  const response = await publicClient.recommenderConversation({
    ...
    body: {
      ...
      context: [
        ...
        { type: 'instruction', value: 'Use only the supplied incident evidence. Do not produce a generic RCA. State clearly when a definitive root cause is not proven.' },
      ],
      annotations: { origin: 'Axis Problem Intelligence RCA', problemId },
    },
  });
...
export default async function (payload: AnalyzePayload) {
  ...
  return {
    ...
    nativeRootCauseEntity: rootCauseEntity || null,
    definitiveRootCause: Boolean(rootCauseEntity),
    analysis: assist.text,
    ...
    evidenceSummary: {
      ...
      relatedProblems: evidence.related.length,
      incidentLogs: evidence.logs.length,
      nearbyDeployments: evidence.deployments.length,
    },
  };
}
