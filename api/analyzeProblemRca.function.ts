...
async function dql(query: string, max = 200): Promise<Row[]> {
  ...
  const queryResult: QueryResult = result;
  return (queryResult.records ?? []).filter((record): record is Row => record !== null);
}
...
async function askAssist(problemId: string, evidence: Evidence): Promise<{ text: string; status: string }> {
  ...
  return { text: responseText.trim(), status: response.status };
}
