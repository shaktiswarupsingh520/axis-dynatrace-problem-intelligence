...
const text = (value: unknown): string => {
  ...
  if (typeof value === 'object') {
    ...
    for (const k of ['name', 'entityName', 'displayName', 'id', 'entityId', 'value']) {
      const v = text(r[k]);
      if (v) return v;
    }
  }
...
function problemFromDql(row: Row, id: string): Row {
  return {
...
function problemFromNative(native: unknown, id: string): Row {
  ...
  return {
    display_id: text(p.displayId) || text(p.display_id) || id,
    'event.name': text(p.title) || text(p.problemTitle) || text(p.eventName) || 'Dynatrace Problem',
    'event.status': text(p.status) || 'UNKNOWN',
    'event.severity': text(p.severityLevel) || text(p.severity) || 'UNKNOWN',
    'event.start': text(p.startTime) || text(p.start),
    'event.end': text(p.endTime) || text(p.end),
    'event.description': text(p.description) || text(p.eventDescription),
    'dt.davis.impact_level': text(p.impactLevel) || text(p.impact),
    root_cause_entity_id: findValue(p, ['rootCauseEntity', 'root_cause_entity_id', 'smartscapeEntity']),
    affected_entity_names: findValue(p, ['affectedEntityNames']),
    affected_users_count: findValue(p, ['affectedUsersCount', 'affected_users_count']),
  };
}
...
async function loadProblem(id: string): Promise<Row> {
  ...
  const rows = await boundedDql(`fetch dt.davis.problems, from:now()-7d, to:now()\n| filter display_id == "${q(id)}"\n| limit 1`, 1);
  return rows[0] ? problemFromDql(rows[0], id) : { display_id: id, 'event.name': 'Dynatrace Problem', 'event.status': 'UNKNOWN', 'event.severity': 'UNKNOWN' };
}
...
async function loadEvidence(id: string): Promise<Evidence> {
  ...
  const [events, logs, history] = await Promise.all([
    eventsQuery ? boundedDql(eventsQuery, 40) : Promise.resolve([] as Row[]),
    logsQuery ? boundedDql(logsQuery, 20) : Promise.resolve([] as Row[]),
...
function fallbackRca(id: string, evidence: Evidence): string {
  ...
  const root = text(p.root_cause_entity_id);
...
export default async function (payload: AnalyzePayload) {
  ...
  const p = evidence.problem;
  const nativeRootCauseEntity = text(p.root_cause_entity_id) || text(evidence.events.find((e) => e['dt.davis.is_rootcause_relevant'] === true)?.root_cause_entity_id) || null;
