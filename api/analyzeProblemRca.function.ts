...
async function loadEvidence(id: string): Promise<Evidence> {
  ...
  const entityIds: string[] = Array.isArray(problem.affected_entity_ids) ? problem.affected_entity_ids.map(text).filter(Boolean) : [text(problem.affected_entity_ids)].filter(Boolean);
  const entityList = [...new Set(entityIds)].slice(0, 50).map((value) => `"${q(value)}"`).join(', ');
  const eventIds: string[] = Array.isArray(problem['dt.davis.event_ids']) ? problem['dt.davis.event_ids'].map(text).filter(Boolean) : [];
  ...
  try {
    const countRows: Row[] = await dql(`fetch dt.davis.problems, from:now()-365d, to:now()\n| filter not(dt.davis.is_duplicate) and event.name == "${eventName}"\n| summarize occurrenceCount=count()`, 5);
