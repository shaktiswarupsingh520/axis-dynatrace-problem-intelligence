...
async function loadEvidence(id: string): Promise<Evidence> {
  ...
  if (eventId) {
    try { snapshots = await dql(`fetch dt.davis.problems.snapshots, from:now()-365d, to:now()\n| filter event.id == "${q(eventId)}"\n| fields timestamp,event.status,event.status_transition,event.severity,event.name,root_cause_entity_id\n| sort timestamp asc\n| limit 50`, 50); } catch (error) { snapshots = []; void error; }
  }
