...
async function askAssist(problemId: string, evidence: Evidence): Promise<{ text: string; status: string }> {
  ...
  const response = await publicClient.recommenderConversation({
    body: {
      ...
      annotations: { origin_app: 'Axis Problem Intelligence', problemId, prompt_version: 'rca-v1' },
    },
  });
  if (Array.isArray(response)) return { text: '', status: 'STREAMING_UNSUPPORTED' };
  if (!response || response.status === 'FAILED' || !response.text) {
    throw new Error(`Dynatrace Assist RCA returned no usable response (status: ${response?.status ?? 'empty'}).`);
  }
