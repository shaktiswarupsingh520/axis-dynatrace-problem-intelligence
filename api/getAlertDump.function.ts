...
function buildQuery(range: string, status: string, severity: string, zone: string): string {
  ...
  return `${query}\n| fieldsAdd problem_duration_minutes = toDouble((coalesce(event.end, now()) - event.start) / 1m)\n| sort event.start desc\n| limit 1000`;
}
...
function transform(row: Row): Row {
  return {
    ...
    'problem.duration': Number.isFinite(Number(text(row.problem_duration_minutes))) ? `${Math.max(0, Number(text(row.problem_duration_minutes))).toFixed(1)} min` : '—',
