...
function buildCausalAnalysis(problem: { title?: string; impactLevel?: string; rootCauseEntity?: { name?: string; entityId?: { id?: string; type?: string } }; evidenceDetails?: { details?: EvidenceLike[] }; impactAnalysis?: { impacts?: ImpactLike[] } }, context?: { problemRecord: Row; eventRecords: Row[] }) {
  ...
  else if (supporting) { probableCause = `Davis did not expose a definitive root-cause entity. Strongest available evidence: ${supporting}. This is evidence, not a confirmed cause.`; remediation = 'Correlate the affected entity, Davis event timeline and incident logs before assigning a definitive root cause.'; confidence = ready === false ? 'Pending Davis analysis' : 'Low'; }
