...
async function executeDql(query: string): Promise<Row[]> {
  try {
    ...
    if (started.result?.records) {
      const records: unknown = started.result.records;
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      if (polled.result?.records) {
        const records: unknown = polled.result.records;
        return Array.isArray(records) ? records.filter((record): record is Row => record !== null && typeof record === 'object' && !Array.isArray(record)) : [];
      }
...
function buildCausalAnalysis(
  problem: { title?: string; impactLevel?: string; rootCauseEntity?: { name?: string; entityId?: { id?: string; type?: string } }; evidenceDetails?: { details?: EvidenceLike[] }; impactAnalysis?: { impacts?: ImpactLike[] } },
  context?: { problemRecord: Row; eventRecords: Row[] },
) {
  const title = text(problem.title) || 'Dynatrace problem';
  const evidence = problem.evidenceDetails?.details ?? [];
  const rootEvidence = evidence.find((item) => item.rootCauseRelevant);
  ...
  const causalEvent = events.find((event) => event['dt.davis.is_rootcause_relevant'] === true) ?? events[0];
  const eventDescriptions = events.map((event) => text(event['event.description'])).filter(Boolean);
  const evidenceText = evidence.map((item) => [text(item.displayName) || text(item.evidenceType), item.entity?.name ? `entity ${item.entity.name}` : '', item.groupingEntity?.name ? `grouped by ${item.groupingEntity.name}` : ''].filter(Boolean).join(' — ')).filter(Boolean);
  ...
  if (rootCauseName) {
    ...
    probableCause = `Davis did not expose a definitive root-cause entity. Strongest available evidence: ${supporting}. This is evidence, not a confirmed cause.`;
  ...
  return {
    description: text(context?.problemRecord['event.description']) || combinedEvidence.join('. ') || title,
    ...
    eventIds: stringArray(context?.problemRecord['dt.davis.event_ids']),
    analysisReady: ready,
    affectedUsers: typeof context?.problemRecord['dt.davis.affected_users_count'] === 'number' ? context.problemRecord['dt.davis.affected_users_count'] : undefined,
    eventNames: [...new Set(events.map((event) => text(event['event.name'])).filter(Boolean))].slice(0, 12),
    causalEvents: events.filter((event) => event['dt.davis.is_rootcause_relevant'] === true).slice(0, 10).map((event) => ({
      id: text(event['event.id']), name: text(event['event.name']), description: text(event['event.description']), entityId: text(event['dt.smartscape_source.id']), entityType: text(event['dt.smartscape_source.type']),
    })),
  };
}
...
async function enhanceWithAssist(problemId: string, context?: { problemRecord: Row; eventRecords: Row[] }) {
  ...
  try {
    ...
    const get = (label: string) => { const match = answer.match(new RegExp(`${label}\\s*:\\s*(.*?)(?=\\n[A-Z ]+\\s*:|$)`, 'is')); return match?.[1]?.trim(); };
