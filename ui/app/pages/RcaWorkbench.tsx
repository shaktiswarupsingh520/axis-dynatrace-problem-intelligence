...
function downloadRcaExcel(result: Result) {
  const headers = ['Problem ID','Native Root Cause','Past Occurrences','Correlated Events','Incident Logs','Timeline Snapshots','RCA'];
  const summary = [result.problemId, result.nativeRootCauseEntity || 'Not proven', result.occurrenceCount, result.evidenceSummary.correlatedEvents, result.evidenceSummary.incidentLogs, result.evidenceSummary.timelineSnapshots, result.analysis];
  const occurrenceHeaders = ['Problem ID','Title','Status','Severity','Started','Ended','Duration'];
  const occurrenceRows = result.occurrences.map((occurrence) => occurrenceHeaders.map((header) => escCsv(header === 'Problem ID' ? occurrence.problemId : header === 'Title' ? occurrence.title : header === 'Status' ? occurrence.status : header === 'Severity' ? occurrence.severity : header === 'Started' ? occurrence.start : header === 'Ended' ? occurrence.end : occurrence.duration)).join(','));
  const csv = `\uFEFF${[headers.map(escCsv).join(','), summary.map(escCsv).join(','), '', occurrenceHeaders.map(escCsv).join(','), ...occurrenceRows].join('\r\n')}`;
...
function downloadRcaPdf(result: Result) {
  ...
  const occurrenceHtml = result.occurrences.slice(0, 50).map((occurrence) => `<tr><td>${escHtml(occurrence.problemId)}</td><td>${escHtml(occurrence.title)}</td><td>${escHtml(occurrence.status)}</td><td>${escHtml(occurrence.severity)}</td><td>${escHtml(occurrence.start)}</td><td>${escHtml(occurrence.duration)}</td></tr>`).join('');
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Axis RCA</title></head><body><h1>Axis Bank — AI-Assisted Incident RCA</h1><p><b>Problem:</b> ${escHtml(result.problemId)}</p><p><b>Native root cause:</b> ${escHtml(result.nativeRootCauseEntity || 'Not proven by available evidence')}</p><p><b>Past occurrences:</b> ${result.occurrenceCount}</p><hr><pre>${escHtml(lines.join('\n'))}</pre><h2>Past Occurrences</h2><table border="1"><thead><tr><th>Problem</th><th>Title</th><th>Status</th><th>Severity</th><th>Start</th><th>Duration</th></tr></thead><tbody>${occurrenceHtml || '<tr><td colspan="6">No occurrence records returned.</td></tr>'}</tbody></table><script>window.onload=function(){window.print();};</script></body></html>`;
  const encoded = `data:text/html;charset=utf-8,${encodeURIComponent(html)}`;
  const w = window.open(encoded, '_blank', 'width=1100,height=900');
  if (!w) return;
}
...
export const RcaWorkbench = () => {
  ...
  return <Flex flexDirection="column" padding={24} gap={16}>
    <div style={{border:'1px solid #dbe3ec',borderRadius:14,background:'#fff',overflow:'hidden'}}>
      <div style={{padding:'22px 24px',background:'#f5f9fd'}}><div style={{fontSize:10,fontWeight:800,letterSpacing:'.14em',color:'#1476d4'}}>DYNATRACE DAVIS + ASSIST</div><Heading>AI Root Cause &amp; RCA</Heading><Paragraph>Evidence-backed analysis using Davis problem data, correlated events, incident logs and recurrence history.</Paragraph></div>
      ...
      <div style={{padding:'0 24px 24px'}}>{error && <div style={{padding:12,border:'1px solid #e5aaa5',background:'#fff5f4',borderRadius:8,color:'#9b241b'}}>RCA could not be generated: {error.message}</div>}
        {data && <><div style={{display:'grid',gridTemplateColumns:'repeat(6,minmax(110px,1fr))',gap:8,margin:'12px 0 18px'}}><div><small>Problem</small><strong>{data.problemId}</strong></div><div><small>Native root cause</small><strong>{data.nativeRootCauseEntity||'Not proven'}</strong></div><div><small>Correlated events</small><strong>{data.evidenceSummary.correlatedEvents}</strong></div><div><small>Incident logs</small><strong>{data.evidenceSummary.incidentLogs}</strong></div><div><small>Timeline snapshots</small><strong>{data.evidenceSummary.timelineSnapshots}</strong></div><div><small>Past occurrences</small><button type="button" onClick={()=>setShowOccurrences(true)}>{data.occurrenceCount}</button></div></div><pre style={{whiteSpace:'pre-wrap',fontSize:12,lineHeight:1.6,border:'1px solid #e2e8ef',borderRadius:10,padding:18,margin:0}}>{data.analysis}</pre>{data.assistFallback&&<div style={{marginTop:10,padding:9,background:'#fff8e8',borderLeft:'4px solid #d89b20',fontSize:11}}>Dynatrace Assist was unavailable for this run. The displayed RCA is generated from retrieved Davis evidence.</div>}</>}
      </div>
    </div>
    {showOccurrences&&data&&<div style={{position:'fixed',inset:0,background:'rgba(7,18,31,.58)',zIndex:10000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={(e)=>{if(e.target===e.currentTarget)setShowOccurrences(false)}}><section style={{width:'min(1050px,94vw)',maxHeight:'88vh',overflow:'auto',background:'#fff',borderRadius:14,padding:20}}><button type="button" onClick={()=>setShowOccurrences(false)}>Close</button><Heading>Past occurrences — {data.occurrenceCount}</Heading><Paragraph>Showing the latest {occurrences.length} occurrence records returned for investigation.</Paragraph><table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}><thead><tr>{['Problem','Title','Status','Severity','Started','Ended','Duration'].map((h)=><th key={h} style={{textAlign:'left',padding:8,borderBottom:'1px solid #e4e9ef'}}>{h}</th>)}</tr></thead><tbody>{occurrences.map((o,i)=><tr key={`${o.problemId}-${i}`}><td>{o.problemId}</td><td>{o.title}</td><td>{o.status}</td><td>{o.severity}</td><td>{o.start}</td><td>{o.end||'—'}</td><td>{o.duration}</td></tr>)}</tbody></table></section></div>}
  </Flex>;
};
