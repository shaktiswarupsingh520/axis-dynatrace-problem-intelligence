...
function reportHtml(result: Result): string {
  ...
  const occurrenceRows = result.occurrences.slice(0, 30).map((o) => `<tr><td>${escapeHtml(o.problemId)}</td><td>${escapeHtml(dt(o.start))}</td><td>${escapeHtml(o.title)}</td><td>${escapeHtml(o.status)}</td><td>${escapeHtml(o.duration)}</td></tr>`).join('');
  const factRow = (label: string, value: string) => `<tr><td class="key">${escapeHtml(label)}</td><td>${escapeHtml(value || 'Not available')}</td></tr>`;
  const block = (title: string, value: string) => `<section class="report-block"><h3>${escapeHtml(title)}</h3><div class="prose">${escapeHtml(value || 'Not available from retrieved evidence.')}</div></section>`;
  const page = (number: number, label: string, content: string, extra = '') => `<article class="page ${extra}"><header><div>AXIS BANK <span>| ApMoSys TECHNOLOGIES</span></div><div>${escapeHtml(label)} <b>${number} / 7</b></div></header><main>${content}</main><footer>Axis Davis Problem Intelligence &nbsp;|&nbsp; AI-assisted incident RCA &nbsp;|&nbsp; Confidential</footer></article>`;
  ...
  const titleBlock = `<div class="section-title"><span></span><h2>Incident at a glance</h2></div>`;

  const cover = `<div class="cover"><div class="cover-brand">AXIS BANK<br><small>ApMoSys TECHNOLOGIES</small></div><div class="cover-kicker">AI-ASSISTED</div><h1>INCIDENT ROOT<br>CAUSE ANALYSIS</h1><div class="cover-id">${escapeHtml(result.problemId)}</div><div class="cover-subtitle">${escapeHtml(facts.title)}</div><div class="cover-rule"></div><div class="brief-label">EXECUTIVE INCIDENT BRIEF</div>${cards([['STATUS', facts.status], ['SEVERITY', `Level ${facts.severity}`], ['DURATION', facts.duration], ['RECURRENCE', String(result.occurrenceCount)]])}<div class="cover-window"><b>Incident window</b><span>${escapeHtml(dt(facts.start))} &nbsp; to &nbsp; ${escapeHtml(facts.end ? dt(facts.end) : '—')}</span><b>Root-cause entity</b><span>${escapeHtml(root)}</span></div><div class="cover-foot">Prepared by Axis Problem Intelligence<br>Generated ${escapeHtml(dt(result.generatedAt))}<span>CONFIDENTIAL</span></div></div>`;

  const p2 = `${titleBlock}<div class="eyebrow">Leadership summary</div><h1 class="page-heading">Executive Decision View</h1><div class="info-grid"><div><div class="small-label">PROBLEM ID</div><b class="blue">${escapeHtml(result.problemId)}</b></div><div><div class="small-label">STATUS</div><b>${escapeHtml(facts.status)}</b></div><div><div class="small-label">SEVERITY</div><b>${escapeHtml(`Level ${facts.severity)}`)}</b></div><div><div class="small-label">CATEGORY</div><b>${escapeHtml(facts.category)}</b></div></div><div class="finding"><div class="small-label">PRIMARY FINDING</div><p>${escapeHtml(executive)}</p></div><div class="two-col"><div>${block('Impact & recurrence', `Past occurrences: ${result.occurrenceCount}\nRecurrence scope: ${result.recurrenceWindow || '30 days'}\nManagement zone: ${scope}\nImpact level: ${facts.impactLevel}\nAffected users: ${facts.affectedUsers}`)}</div><div><section class="report-block"><h3>Incident facts</h3><table>${factRow('Incident', facts.title)}${factRow('Root-cause entity', root)}${factRow('Started', facts.start ? dt(facts.start) : 'Not available')}${factRow('Ended', facts.end ? dt(facts.end) : '—')}${factRow('Affected users', facts.affectedUsers)}${factRow('Impact level', facts.impactLevel)}</table></section></div></div>`;

  const p3 = `${titleBlock}<div class="eyebrow">Evidence and causal chain</div><h1 class="page-heading">Root Cause Assessment</h1>${block('Evidence-backed interpretation', rootAssessment || `Native Dynatrace root-cause entity: ${root}. The retrieved evidence supports the incident signal; any underlying trigger not explicitly identified by Davis remains unproven.`)}<div class="chain"><h3>Technical root-cause chain</h3><pre>${escapeHtml(chain || '[Evidence retrieved]\n        ↓\n[Observed problem signal]\n        ↓\n[Service impact]\n        ↓\n[Root cause: not proven unless identified by Davis]')}</pre></div><div class="legend"><b>Proven / high confidence</b><span>Retrieved directly from Dynatrace evidence.</span><b>Inference / validation required</b><span>AI interpretation that requires SRE validation.</span><b>Evidence gap</b><span>Telemetry not present in the retrieved evidence.</span></div>`;

  const p4 = `${titleBlock}<div class="eyebrow">Operational history</div><h1 class="page-heading">Incident Timeline & Recurrence</h1><div class="timeline-cards">${cards([['DETECTION / START', facts.start ? dt(facts.start) : 'Not available'], ['RECOVERY / END', facts.end ? dt(facts.end) : '—'], ['DURATION', facts.duration], ['RECURRENCE', String(result.occurrenceCount)]])}</div>${block('Incident timeline', timeline || `Davis problem opened at ${facts.start ? dt(facts.start) : 'the supplied start time'} and is ${facts.end ? 'closed' : 'ongoing / without an end timestamp in the retrieved data'}.`)}<section class="report-block"><h3>Recurrence pattern</h3><p><b>${result.occurrenceCount}</b> matching occurrence(s) found in the last ${result.recurrenceWindow === '30d' ? '30 days' : escapeHtml(result.recurrenceWindow || '30 days')} within ${escapeHtml(scope)}.</p><table class="occurrences"><thead><tr><th>Problem ID</th><th>Started</th><th>Title</th><th>Status</th><th>Duration</th></tr></thead><tbody>${occurrenceRows || '<tr><td colspan="5">No occurrence records returned.</td></tr>'}</tbody></table></section>`;

  const p5 = `${titleBlock}<div class="eyebrow">Operational response plan</div><h1 class="page-heading">Remediation & Preventive Actions</h1>${block('Immediate stabilization', immediate)}${block('Permanent / preventive controls', preventive)}${block('Monitoring recommendations', monitoring)}<div class="governance">Recommendations are proposals only. Validate ownership, feasibility and production impact with the responsible SRE/application teams before implementation.</div>`;

  const p6 = `${titleBlock}<div class="eyebrow">RCA quality controls</div><h1 class="page-heading">Validation, Confidence & Governance</h1>${block('Validation checklist', validation || 'Confirm root cause; validate dependency chain; correlate infrastructure metrics; test remediation; monitor recovery; validate alerting; document findings; communicate closure.')}<section class="confidence"><div class="confidence-label">CONFIDENCE STATEMENT</div><h3>Evidence strength</h3><p>${escapeHtml(confidence || 'Confidence should reflect the strength of the supplied Dynatrace evidence. Any unproven trigger must remain explicitly identified as an evidence gap.')}</p></section><div class="governance"><b>Governance note</b><br>Observed Dynatrace evidence is distinguished from AI-generated interpretation. Recommendations require validation and ownership before being treated as completed remediation.</div>`;
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Axis CIO RCA ${escapeHtml(result.problemId)}</title><style>
@page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#172334;font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.45}.page{width:210mm;min-height:297mm;position:relative;page-break-after:always;background:#fff;padding:23mm 16mm 18mm}.page:last-child{page-break-after:auto}.page>header{position:absolute;left:16mm;right:16mm;top:9mm;display:flex;justify-content:space-between;font-size:8px;font-weight:800;color:#17345d;border-bottom:1px solid #dbe3ec;padding-bottom:5px}.page>header span{font-weight:500;color:#718096}.page>header b{margin-left:12px;color:#17345d}.page>main{min-height:255mm}.page>footer{position:absolute;left:16mm;right:16mm;bottom:8mm;border-top:1px solid #dbe3ec;padding-top:4px;font-size:7.5px;color:#718096;display:flex;justify-content:space-between}.cover{height:297mm;background:#14284a;color:#fff;padding:22mm 17mm 15mm;position:relative;overflow:hidden}.cover:before{content:'';position:absolute;left:0;top:0;bottom:0;width:5mm;background:#2f62d0}.cover-brand{font-size:13px;font-weight:800;line-height:1.2}.cover-brand small{font-size:8px;font-weight:500;letter-spacing:.04em}.cover-kicker{margin-top:42mm;color:#72a9ff;font-weight:800;letter-spacing:.18em;font-size:9px}.cover h1{font-size:28px;line-height:1.02;letter-spacing:.02em;margin:4mm 0 7mm}.cover-id{font-size:13px;font-weight:800}.cover-subtitle{font-size:10px;color:#91a1b8;margin-top:3mm}.cover-rule{height:1px;background:#9aa9bf;margin:27mm 0 7mm}.brief-label{font-size:8px;font-weight:800;letter-spacing:.08em;color:#72a9ff}.cards{display:grid;grid-template-columns:repeat(4,1fr);gap:4mm;margin:6mm 0}.metric{background:#f4f7fb;color:#172334;border-radius:5px;padding:4mm;min-height:18mm}.metric span{display:block;font-size:7px;color:#718096;font-weight:800;letter-spacing:.06em}.metric strong{display:block;margin-top:2mm;font-size:11px;overflow-wrap:anywhere}.cover .metric:first-child{background:#f26a6a;color:#fff}.cover .metric:first-child span{color:#fff}.cover-window{display:grid;grid-template-columns:34mm 1fr;gap:3mm 5mm;margin-top:9mm;font-size:8.5px}.cover-window b{color:#91a1b8;font-size:7px;text-transform:uppercase}.cover-window span{font-weight:700}.cover-foot{position:absolute;left:17mm;right:17mm;bottom:14mm;color:#91a1b8;font-size:7.5px}.cover-foot span{float:right}.section-title{display:flex;align-items:center;gap:3mm;margin-bottom:2mm}.section-title span{width:2mm;height:7mm;border-radius:1mm;background:#2f62d0}.section-title h2{margin:0;font-size:13px;color:#172f53}.eyebrow{font-size:8px;color:#718096;margin:2mm 0}.page-heading{font-size:20px;color:#172f53;margin:1mm 0 7mm}.small-label{font-size:7px;color:#64748b;font-weight:800;letter-spacing:.06em}.info-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm;margin-bottom:5mm}.info-grid>div{border:1px solid #dbe3ec;border-radius:4px;padding:3.5mm;background:#f7f9fc;min-height:15mm}.info-grid b{display:block;margin-top:2mm;font-size:10px}.blue{color:#2861c7}.finding{border:1px solid #dbe3ec;background:#f7f9fc;border-radius:5px;padding:5mm;margin-bottom:5mm}.finding p{margin:2mm 0 0;font-size:9.5px}.two-col{display:grid;grid-template-columns:1fr 1fr;gap:5mm}.report-block{margin:5mm 0;break-inside:avoid}.report-block h3,.chain h3{font-size:11px;color:#17345d;border-bottom:1px solid #d9e3ef;padding-bottom:2mm;margin:0 0 3mm}.prose{white-space:pre-wrap;font-size:9px;color:#27384d}.report-block table{width:100%;border-collapse:collapse;font-size:8px}.report-block td{padding:2mm;border-bottom:1px solid #e4e9ef;vertical-align:top}.report-block td.key{width:36%;font-weight:700;color:#4b5e73}.chain{background:#f7f9fc;border:1px solid #dbe3ec;border-radius:5px;padding:4mm;margin-top:6mm}.chain pre{font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap;margin:0;font-size:9px;line-height:1.65;color:#263d5c}.legend{display:grid;grid-template-columns:45mm 1fr;gap:2mm 4mm;margin-top:6mm;padding:4mm;background:#fbfcfe;border:1px solid #e0e6ed;border-radius:5px;font-size:8px}.legend b{color:#17345d}.timeline-cards .cards{margin-top:2mm}.occurrences th{background:#17345d;color:#fff;text-align:left;padding:2mm;font-size:7px}.occurrences td{padding:2mm;border-bottom:1px solid #dfe5ec}.occurrences tr:nth-child(even){background:#f7f9fc}.governance{margin-top:6mm;padding:4mm;background:#fff8e7;border-left:3px solid #d49b25;font-size:8px;color:#4b3b1a;break-inside:avoid}.confidence{margin-top:8mm;padding:6mm;border:1px solid #cbd8e6;border-radius:5px;background:#f7f9fc}.confidence-label{font-size:7px;color:#2f62d0;font-weight:800;letter-spacing:.08em}.confidence h3{margin:2mm 0;font-size:12px;color:#17345d}.confidence p{margin:0;font-size:9px;white-space:pre-wrap}.appendix-title{border-bottom:2px solid #17345d;padding-bottom:5mm;margin-bottom:5mm}.appendix-title h1{font-size:22px;color:#17345d;margin:1mm 0}.appendix-meta{font-size:8px;color:#718096}.appendix-analysis{font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap;font-size:8.5px;line-height:1.48;color:#26384c}table{page-break-inside:auto}tr{page-break-inside:avoid;page-break-after:auto}@media print{body{print-color-adjust:exact;-webkit-print-color-adjust:exact}.page{box-shadow:none}}</style></head><body>${page(1, 'Executive Incident Brief', cover, 'cover-page')}${page(2, 'Leadership summary', p2)}${page(3, 'Evidence and causal chain', p3)}${page(4, 'Operational history', p4)}${page(5, 'Operational response plan', p5)}${page(6, 'RCA quality controls', p6)}${page(7, 'Detailed AI-generated RCA', p7)}</body></html>`;
}
...
function printRca(result: Result): void {
  ...
  const print = () => {
    try { windowRef.focus(); windowRef.print(); } catch (error) { void error; }
  };
