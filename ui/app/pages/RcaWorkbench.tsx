...
const CSS = [
  ...
  'html,body{margin:0;padding:0;background:#fff;color:#172334;font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.45}',
  ...
  '.page>header span{font-weight:500;color:#718096}', '.page>header b{margin-left:12px;color:#17345d}', '.page>main{min-height:255mm}',
  ...
  '.cover-page{width:210mm;height:297mm;page-break-after:always;background:#14284a}', '.cover{height:297mm;background:#14284a;color:#fff;padding:22mm 17mm 15mm;position:relative;overflow:hidden}',
  '.cover:before{content:"";position:absolute;left:0;top:0;bottom:0;width:5mm;background:#2f62d0}', '.cover-brand{font-size:13px;font-weight:800;line-height:1.2}',
  '.cover-brand small{font-size:8px;font-weight:500;letter-spacing:.04em}', '.cover-kicker{margin-top:42mm;color:#72a9ff;font-weight:800;letter-spacing:.18em;font-size:9px}',
  '.cover h1{font-size:28px;line-height:1.02;letter-spacing:.02em;margin:4mm 0 7mm}', '.cover-id{font-size:13px;font-weight:800}', '.cover-subtitle{font-size:10px;color:#91a1b8;margin-top:3mm}',
  '.cover-rule{height:1px;background:#9aa9bf;margin:27mm 0 7mm}', '.brief-label{font-size:8px;font-weight:800;letter-spacing:.08em;color:#72a9ff}',
  '.cards{display:grid;grid-template-columns:repeat(4,1fr);gap:4mm;margin:6mm 0}', '.metric{background:#f4f7fb;color:#172334;border-radius:5px;padding:4mm;min-height:18mm}',
  '.metric span{display:block;font-size:7px;color:#718096;font-weight:800;letter-spacing:.06em}', '.metric strong{display:block;margin-top:2mm;font-size:11px;overflow-wrap:anywhere}',
  '.cover .metric:first-child{background:#f26a6a;color:#fff}', '.cover .metric:first-child span{color:#fff}',
  '.cover-window{display:grid;grid-template-columns:34mm 1fr;gap:3mm 5mm;margin-top:9mm;font-size:8.5px}', '.cover-window b{color:#91a1b8;font-size:7px;text-transform:uppercase}', '.cover-window span{font-weight:700}',
  '.cover-foot{position:absolute;left:17mm;right:17mm;bottom:14mm;color:#91a1b8;font-size:7.5px}', '.cover-foot span{float:right}',
  '.section-title{display:flex;align-items:center;gap:3mm;margin-bottom:2mm}', '.section-title span{width:2mm;height:7mm;border-radius:1mm;background:#2f62d0}', '.section-title h2{margin:0;font-size:13px;color:#172f53}',
  '.eyebrow{font-size:8px;color:#718096;margin:2mm 0}', '.page-heading{font-size:20px;color:#172f53;margin:1mm 0 7mm}', '.small-label{font-size:7px;color:#64748b;font-weight:800;letter-spacing:.06em}',
  '.info-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm;margin-bottom:5mm}', '.info-grid>div{border:1px solid #dbe3ec;border-radius:4px;padding:3.5mm;background:#f7f9fc;min-height:15mm}', '.info-grid b{display:block;margin-top:2mm;font-size:10px}', '.blue{color:#2861c7}',
  '.finding{border:1px solid #dbe3ec;background:#f7f9fc;border-radius:5px;padding:5mm;margin-bottom:5mm}', '.finding p{margin:2mm 0 0;font-size:9.5px}', '.two-col{display:grid;grid-template-columns:1fr 1fr;gap:5mm}',
  '.report-block{margin:5mm 0;break-inside:avoid}', '.report-block h3,.chain h3{font-size:11px;color:#17345d;border-bottom:1px solid #d9e3ef;padding-bottom:2mm;margin:0 0 3mm}', '.prose{white-space:pre-wrap;font-size:9px;color:#27384d}',
  '.report-block table{width:100%;border-collapse:collapse;font-size:8px}', '.report-block td{padding:2mm;border-bottom:1px solid #e4e9ef;vertical-align:top}', '.report-block td.key{width:36%;font-weight:700;color:#4b5e73}',
  '.chain{background:#f7f9fc;border:1px solid #dbe3ec;border-radius:5px;padding:4mm;margin-top:6mm}', '.chain pre{font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap;margin:0;font-size:9px;line-height:1.65;color:#263d5c}',
  '.legend{display:grid;grid-template-columns:45mm 1fr;gap:2mm 4mm;margin-top:6mm;padding:4mm;background:#fbfcfe;border:1px solid #e0e6ed;border-radius:5px;font-size:8px}', '.legend b{color:#17345d}', '.timeline-cards .cards{margin-top:2mm}',
  '.occurrences th{background:#17345d;color:#fff;text-align:left;padding:2mm;font-size:7px}', '.occurrences td{padding:2mm;border-bottom:1px solid #dfe5ec}', '.occurrences tr:nth-child(even){background:#f7f9fc}',
  '.governance{margin-top:6mm;padding:4mm;background:#fff8e7;border-left:3px solid #d49b25;font-size:8px;color:#4b3b1a;break-inside:avoid}', '.confidence{margin-top:8mm;padding:6mm;border:1px solid #cbd8e6;border-radius:5px;background:#f7f9fc}',
  '.confidence-label{font-size:7px;color:#2f62d0;font-weight:800;letter-spacing:.08em}', '.confidence h3{margin:2mm 0;font-size:12px;color:#17345d}', '.confidence p{margin:0;font-size:9px;white-space:pre-wrap}',
  '.appendix-title{border-bottom:2px solid #17345d;padding-bottom:5mm;margin-bottom:5mm}', '.appendix-title h1{font-size:22px;color:#17345d;margin:1mm 0}', '.appendix-meta{font-size:8px;color:#718096}',
  '.appendix-analysis{font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap;font-size:8.5px;line-height:1.48;color:#26384c}', 'table{page-break-inside:auto}', 'tr{page-break-inside:avoid;page-break-after:auto}',
...
function reportHtml(result: Result): string {
  ...
  const page = (number: number, label: string, content: string) => `<article class="page"><header><div>AXIS BANK <span>| ApMoSys TECHNOLOGIES</span></div><div>${escapeHtml(label)} <b>${number} / 7</b></div></header><main>${content}</main><footer>Axis Davis Problem Intelligence <span>| AI-assisted incident RCA | Confidential</span></footer></article>`;
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Axis CIO RCA ${escapeHtml(result.problemId)}</title><style>${CSS}</style></head><body>${cover}${page(2, 'Leadership summary', p2)}${page(3, 'Evidence and causal chain', p3)}${page(4, 'Operational history', p4)}${page(5, 'Operational response plan', p5)}${page(6, 'RCA quality controls', p6)}${page(7, 'Detailed AI-generated RCA', p7)}</body></html>`;
}
