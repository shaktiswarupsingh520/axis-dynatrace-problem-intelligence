...
const download = (content: string, type: string, name: string) => { const url = URL.createObjectURL(new Blob([content], { type })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = name; anchor.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); };
const escCsv = (value: unknown) => `"${text(value).replace(/"/g, '""')}"`;

export const AlertDump = () => {
  const navigate = useNavigate();
  const [range, setRange] = useState('24h'); const [status, setStatus] = useState('ALL'); const [severity, setSeverity] = useState('ALL'); const [zoneId, setZoneId] = useState('ALL'); const [page, setPage] = useState(1); const pageSize = 50;
  ...
  const rows = useMemo(() => query.data?.rows ?? [], [query.data]); const zones = useMemo(() => query.data?.managementZones ?? [], [query.data]);
  const pageCount = Math.max(1, Math.ceil(rows.length / pageSize)); const currentPage = Math.min(page, pageCount); const visible = useMemo(() => rows.slice((currentPage - 1) * pageSize, currentPage * pageSize), [rows, currentPage]);
  ...
  return <Flex flexDirection="column" padding={24} gap={16}>
    <div style={{ border: '1px solid #dbe3ec', borderRadius: 14, background: '#fff', overflow: 'hidden' }}>
      <div style={{ padding: '20px 24px', background: '#f5f9fd' }}><Heading>Dynatrace Alert Dump</Heading><Paragraph>Live Davis problems with Management Zone, status, severity and time filtering.</Paragraph></div>
      <div style={{ display: 'flex', gap: 10, alignItems: 'end', flexWrap: 'wrap', padding: '15px 24px', borderBlock: '1px solid #e2e8ef', background: '#f7f9fb' }}>
        ...
        <button type="button" onClick={() => download(csv(), 'text/csv;charset=utf-8', 'dynatrace-alert-dump.csv')} disabled={!rows.length}>Download Excel</button>
