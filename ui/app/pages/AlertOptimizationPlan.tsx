...
const mins = (n: number) =>
  n < 60 ? `${n.toFixed(0)}m` : `${Math.floor(n / 60)}h ${Math.round(n % 60)}m`;
...
export const AlertOptimizationPlan = () => {
  ...
  const generate = async () => {
    ...
    try {
      const r = await fetch('/api/getAlertOptimizationPlan', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ managementZoneName: zone }),
      });
      const body = await r.text();
      if (!r.ok) throw new Error(body || `Alert Optimization Plan failed with HTTP ${r.status}`);
  ...
  const assist = (value: string) =>
    value.split(/\r?\n/).map((line, i) => (
      <p key={i} className={/^\s*(?:#{1,6}|\d+\.)/.test(line) ? 'aop-heading-line' : ''}>
        {line || ' '}
      </p>
    ));
      ...
      <div className="aop-shell">
        <header className="aop-header">
          <div>
            ...
            <p>
              Analyze the last 30 days of Davis problems for recurring noise, threshold-review
              candidates, immediate actions and optimization opportunities.
            </p>
          </div>
          <div className="aop-window">LOOKBACK<strong>30 DAYS</strong></div>
        </header>
        ...
        <section className="aop-filter">
          <div className="aop-picker">
            ...
            <button
              type="button"
              className="aop-trigger"
              onClick={() => {
                setOpen(!open);
                if (!zones.length) void loadZones();
                setSearch('');
              }}
            >
              <span>{zone || 'Select Management Zone'}</span>
              <span>⌄</span>
            </button>
            {open && (
              <div className="aop-menu">
                <input
                  autoFocus
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  placeholder="Search management zones…"
                />
                <div className="aop-options">
                  {filtered.map(z => (
                    <button key={z.id} type="button" onClick={() => { setZone(z.name); setOpen(false); }}>
                      {z.name}
                    </button>
                  ))}
          ...
          <button
            type="button"
            className="aop-generate"
            disabled={!zone || loading}
            onClick={() => void generate()}
          >
            {loading ? 'Analysing 30-day data…' : 'Generate Optimization Plan'}
          </button>

          <div className="aop-filter-note">
            Source: Dynatrace Problems API · 30-day history · recommendations: Dynatrace Assist
          </div>
        </section>
        ...
        {loading && (
          <section className="aop-loading">
            <div className="aop-spinner" />
            <div>
              <strong>Building alert optimization intelligence</strong>
              <p>Correlating recurring patterns and asking Dynatrace Assist for recommendations…</p>
            </div>
          </section>
        )}
          ...
          <div className="aop-content">
            ...
            <section className="aop-assist">
              <div className="aop-section-head">
                <div>
                  <div className="aop-kicker">DYNATRACE ASSIST</div>
                  <h2>AI-assisted optimization recommendations</h2>
                </div>
                <span className={plan.assistAnalysis ? 'aop-status-good' : 'aop-status-warn'}>{plan.assistStatus}</span>
              </div>
              <div className="aop-assist-body">
                {plan.assistAnalysis ? assist(plan.assistAnalysis) : <p>Assist recommendations were not returned. Use the evidence tables below.</p>}
              </div>
            </section>
            ...
            <section className="aop-grid">
              <article className="aop-card">
                <div className="aop-section-head">
                  <div><div className="aop-kicker">NOISE / RECURRENCE</div><h2>Repeated alert patterns</h2></div>
                  <span>{plan.totals.recurringPatterns} patterns</span>
                </div>
                <div className="aop-table-wrap">
                  <table>
                    <thead><tr><th>Pattern</th><th>Root Cause</th><th>Count</th><th>Open</th><th>Avg Duration</th><th>Severity</th></tr></thead>
                    <tbody>
              ...
              <article className="aop-card">
                <div className="aop-section-head">
                  <div><div className="aop-kicker">THRESHOLD / SENSITIVITY</div><h2>Review candidates</h2></div>
                  <span>{plan.thresholdCandidates.length} candidates</span>
                </div>
                ...
                <div className="aop-list">
                  {plan.thresholdCandidates.slice(0, 12).map(p => (
                    <div className="aop-list-row" key={p.key}>
                      <div><b>{p.title}</b><small>{p.rootCauseEntity} · {p.occurrences} occurrences · avg {mins(p.avgDurationMinutes)}</small></div>
                      <span>Review</span>
                    </div>
                  ))}
            ...
            <section className="aop-card">
              <div className="aop-section-head">
                <div><div className="aop-kicker">ACTION QUEUE</div><h2>Immediate attention candidates</h2></div>
                <span>{plan.immediateActions.length} candidates</span>
              </div>
              <div className="aop-table-wrap">
                <table>
                  <thead><tr><th>Priority Pattern</th><th>Reason</th><th>Open</th><th>Occurrences</th><th>Avg Duration</th><th>Root Cause</th></tr></thead>
                  <tbody>
            ...
            <section className="aop-card">
              <div className="aop-section-head">
                <div><div className="aop-kicker">METHOD & GOVERNANCE</div><h2>How the plan is generated</h2></div>
              </div>
              <div className="aop-method-grid">
                {plan.methodology.map((m, i) => (
                  <div key={i}><strong>{String(i + 1).padStart(2, '0')}</strong><p>{m}</p></div>
                ))}
              </div>
            </section>
          </div>
