...
export function buildCioRcaHtml(result: CioRcaResult): string {
  ...
  const occurrenceRows = (result.occurrences ?? []).slice(0, 50).map((o) => '<tr><td>' + escapeText(o.problemId) + '</td><td>' + escapeText(o.title) + '</td><td>' + escapeText(o.status) + '</td><td>' + escapeText(o.severity) + '</td><td>' + escapeText(o.duration) + '</td></tr>').join('');
  ...
  const metricHtml = metrics.map(([label,value]) => '<div><div class="label">' + escapeText(String(label)) + '</div><div class="value">' + escapeText(String(value)) + '</div></div>').join('');
