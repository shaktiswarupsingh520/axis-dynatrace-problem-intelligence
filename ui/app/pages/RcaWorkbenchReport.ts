...
function pdfPage(lines: string[], pageNumber: number, totalPages: number, firstPage: boolean): string {
  ...
  let y = 774;
  if (firstPage) {
    commands.push('BT /F2 18 Tf 40 778 Td (Evidence-First Incident RCA) Tj ET');
    y = 744;
  }
...
export function buildCioRcaPdf(result: CioRcaResult): Blob {
  ...
  const occurrences = (result.occurrences ?? []).slice(0, 60);
  if (!occurrences.length) {
    all.push('No occurrence records returned.');
  } else {
    for (const occurrence of occurrences) {
      all.push(...wrap(
        occurrence.problemId + ' | ' + dateText(occurrence.start) + ' | ' + occurrence.title +
        ' | ' + occurrence.status + ' | Sev ' + occurrence.severity + ' | ' + occurrence.duration,
        82
      ));
    }
  }
